const DataProject = [
  {
    title: "Al Qur'an dengan Public API 📖",
    desc: "Ini adalah sebuah project yang dimulai dari keinginan untuk bisa melakukan copy ayat Al-Qur'an dengan mudah, dari gagasan ide itu kemudian dikembangkan menjadi sebuah website full front end dengan menggunaan Library ✨React JS and 🪁TailwindCSS dengan bantuan API.",
    img: "https://images.unsplash.com/photo-1598777025922-76f01b057927?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=387&q=80",
    mt: "January",
    yr: "2023",
    ct: "Personal",
    link: "https://al-quran.pages.dev/",
  },
  {
    title: "menulisapaini fajriyan blog 📝",
    desc: "Ini adalah sebuah project personal blog milik saya pribadi yang saya jadikan menjadi tempat menulis tentang hal apapun yang saya sukai dan tempat saya menyimpan tips dan trik yang saya ketahui, blog ini dibangun menggunakan 📚Blogspot milik Google.",
    img: "https://images2.imgbox.com/4c/33/hC6mUfhQ_o.png",
    mt: "Februari",
    yr: "2023",
    ct: "Personal",
    link: "https://menulisapaini.blogspot.com/",
  },
  {
    title: "Manajemen FEB UNIV. Brawijaya 🎓",
    desc: "Manajemen FEB merupakan project Website Kampus dari Sekawan Sekawan Studio yang saya terlibat dalam pengerjaanya mulai dari awal, Project ini dibangun menggunakan 🌍Wordpress, project ini bisa disebut multi site dikarenakan didalamnya berisi 5 Program Studi dari FEB UB.",
    img: "https://sekawanstudio.com/wp-content/uploads/2022/10/POTTOFOLIO-MANAJEMEN-FEB.webp",
    mt: "Juli",
    yr: "2022",
    ct: "Sekawan Studio",
    link: "https://sekawanstudio.com/portofolio/website-manajemen-feb-ub/",
  },
  {
    title: "Adil Makmur Fajar 🌱",
    desc: "PT Adil Makmur Fajar (AMCO) merupakan perusahaan gabungan antara swasta nasional dan Sundat Co. Ltd Singapura, ini adalah project dari sekawan Sekawan Studio yang saya terlibat dalam pengerjaanya awal sampai live. Pada project ini menggunakan multi language, terdapat 3 bahasa pada websitenya.",
    img: "https://sekawanstudio.com/wp-content/uploads/2022/10/POTTOFOLIO-ADIL-MAKMUR.webp",
    mt: "Agustus",
    yr: "2022",
    ct: "Sekawan Studio",
    link: "https://sekawanstudio.com/portofolio/website-adil-makmur-fajar/",
  },
  {
    title: "FIA Universitas Brawijaya 🎓",
    desc: "Fakultas Ilmu Administrasi Universitas Brawijaya ini adalah project multi site yang besar karena didalam website ini tedapat 3 Departemen dengan lebih dari 10 program studi pada website ini, project ini merupakan project dari sekawan Sekawan Studio yang dalam pengerjaanya saya terlibat didalamnya.",
    img: "https://sekawanstudio.com/wp-content/uploads/2022/10/POTTOFOLIO-FIA.webp",
    mt: "April",
    yr: "2022",
    ct: "Sekawan Studio",
    link: "https://sekawanstudio.com/portofolio/website-fia-ub/",
  },
  {
    title: "Cardion UIN 🏅",
    desc: "Cardion merupakan webite yang digunakan sebagai landing page pendaftaran lomba oleh Universitas Islam Negeri Malang, project ini hanya development saja jadi tanpa UI UX. Website ini merupakan project milik sekawan Sekawan Studio yang dibangun menggunakaan Wordpress.",
    img: "https://sekawanstudio.com/wp-content/uploads/2022/10/POTTOFOLIO-CARDION.webp",
    mt: "Oktober",
    yr: "2022",
    ct: "Sekawan Studio",
    link: "https://sekawanstudio.com/portofolio/website-cardion-uin/",
  },
  {
    title: "Sekawan Sekawan Studio 🏢",
    desc: "Sekawan Sekawan Studio merupakan project internal milik sekawan Sekawan Studio sendiri dimana project ini sebenarnya adalah project Revamp dari website yang sebelumnya, project ini berjalan pada akhir tahun 2022, dalam pengerjaanya saya terlibat dominan sebagai developer yang membuat website ini mulai dari awal.",
    img: "https://thumbs2.imgbox.com/b3/9b/NlLrsTkX_t.png",
    mt: "November",
    yr: "2022",
    ct: "Sekawan Studio",
    link: "https://sekawanstudio.com/",
  },
  {
    title: "Asbes Adimas 🏠",
    desc: "Project asbes adimas adalah salah satu project company profile milik sekawan Sekawan Studio yang saya terlibat dalam pengerjaan. Website ini dibangun dengan mengguanakan Wordpress dengan custom fitur pada websitenya, website ini ditujukan untuk menampilkan beberapa produk dari adimas. ",
    img: "https://sekawanstudio.com/wp-content/uploads/2022/10/POTTOFOLIO-ADIMAS.webp",
    mt: "Desember",
    yr: "2022",
    ct: "Sekawan Studio",
    link: "https://sekawanstudio.com/portofolio/website-adimas/",
  },
];
export default DataProject;
export const SummaryProject = [
  {
    title: "Al Qur'an Dengan Public API 📖",
    desc: "Ini adalah sebuah project yang dimulai dari keinginan untuk bisa melakukan copy ayat Al-Qur'an dengan mudah, dari gagasan ide itu kemudian dikembangkan menjadi sebuah website full front end dengan menggunaan Library ✨React JS and 🪁TailwindCSS dengan bantuan API.",
    img: "https://images.unsplash.com/photo-1598777025922-76f01b057927?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=387&q=80",
    mt: "january",
    yr: "2023",
    ct: "Pribadi",
    link: "https://al-quran.pages.dev/",
  },
  {
    title: "Sekawan Sekawan Studio 🏢",
    desc: "Sekawan Sekawan Studio merupakan project internal milik sekawan Sekawan Studio sendiri dimana project ini sebenarnya adalah project Revamp dari website yang sebelumnya, project ini berjalan pada akhir tahun 2022, dalam pengerjaanya saya terlibat dominan sebagai developer yang membuat website ini mulai dari awal.",
    img: "https://thumbs2.imgbox.com/b3/9b/NlLrsTkX_t.png",
    mt: "November",
    yr: "2022",
    ct: "Sekawan Studio",
    link: "https://sekawanstudio.com/",
  },
];
export const DataChangeLog = [
  /*  color documentation
  success : bg-emerald-100 text-emerald-800
  update : bg-amber-100 text-amber-800 
  error   : bg-red-100 text-red-700
  */
  {
    author: "fajriyan",
    date: "07/03/2023",
    version: "1.0",
    color: "bg-amber-100 text-amber-800",
    status: "update",
    desc: "add data on my project",
  },
  {
    author: "fajriyan",
    date: "09/02/2023",
    version: "1.0",
    color: "bg-amber-100 text-amber-800",
    status: "update",
    desc: "add google site verification",
  },
  {
    author: "fajriyan",
    date: "27/01/2023",
    version: "1.0",
    color: "bg-amber-100 text-amber-800",
    status: "update",
    desc: "add some data expertise",
  },
  {
    author: "fajriyan",
    date: "20/01/2023",
    version: "1.0",
    color: "bg-emerald-100 text-emerald-800",
    status: "release",
    desc: "first website release",
  },
];
export const DataLearnings = [
  {
    id: 1,
    status: "University",
    name: "Universitas PGRI Kanjuruhan Malang",
    major: "S1 Teknik Informatika",
    since: "From 2018 - 2022",
  },
  {
    id: 2,
    status: "Senior High School",
    name: "Sekolah Menengah Akhir 6 Lamongan",
    major: "Matematika dan Ilmu Alam",
    since: "From 2016 - 2018",
  },
  {
    id: 3,
    status: "Junior High School",
    name: "Sekolah Menengah Pertama 14 Lamongan",
    major: "General",
    since: "From 2014 - 2016",
  },
];
export const DataExp = [
  {
    id: 1,
    img: "https://cdn.pixabay.com/photo/2017/08/05/11/16/logo-2582748_640.png",
    name: "HTML 5",
    level: "Advanced ",
  },
  {
    id: 2,
    img: "https://mitacademys.com/wp-content/uploads/2022/07/logo-2582747_1920.png",
    name: "CSS 3",
    level: "Advanced ",
  },
  {
    id: 3,
    img: "https://upload.wikimedia.org/wikipedia/commons/6/6a/JavaScript-logo.png",
    name: "Javascript",
    level: "Intermediate ",
  },
  {
    id: 4,
    img: "https://cdn4.iconfinder.com/data/icons/logos-3/600/React.js_logo-512.png",
    name: "React JS",
    level: "Intermediate ",
  },
  {
    id: 5,
    img: "https://www.websites24-7.co.uk/wp-content/uploads/2015/09/wplogoblue-notext-rgb.png",
    name: "Wordpress",
    level: "Advanced",
  },
  {
    id: 6,
    img: "https://histla.com/wp-content/uploads/2015/02/Codeigniter.png",
    name: "Codeigniter",
    level: "Intermediate",
  },
  {
    id: 7,
    img: "https://qnp.co.id/wp-content/uploads/2022/06/laravel.jpg",
    name: "Laravel",
    level: "Beginner",
  },
  {
    id: 8,
    img: "https://play-lh.googleusercontent.com/PCpXdqvUWfCW1mXhH1Y_98yBpgsWxuTSTofy3NGMo9yBTATDyzVkqU580bfSln50bFU",
    name: "Github",
    level: "Intermediate",
  },
  {
    id: 9,
    img: "https://getbootstrap.com/docs/5.3/assets/brand/bootstrap-logo-shadow.png",
    name: "Bootstrap",
    level: "Advanced",
  },
  {
    id: 10,
    img: "https://upload.wikimedia.org/wikipedia/commons/thumb/d/d5/Tailwind_CSS_Logo.svg/2048px-Tailwind_CSS_Logo.svg.png",
    name: "Tailwind",
    level: "Intermediate",
  },
  {
    id: 11,
    img: "https://i.pinimg.com/originals/66/8c/cc/668cccb3f734f342e07c0185e6d9a975.png",
    name: "Figma",
    level: "Beginner",
  },
];
export const ContactBTN = [
  {
    svg: "M0 1.146C0 .513.526 0 1.175 0h13.65C15.474 0 16 .513 16 1.146v13.708c0 .633-.526 1.146-1.175 1.146H1.175C.526 16 0 15.487 0 14.854V1.146zm4.943 12.248V6.169H2.542v7.225h2.401zm-1.2-8.212c.837 0 1.358-.554 1.358-1.248-.015-.709-.52-1.248-1.342-1.248-.822 0-1.359.54-1.359 1.248 0 .694.521 1.248 1.327 1.248h.016zm4.908 8.212V9.359c0-.216.016-.432.08-.586.173-.431.568-.878 1.232-.878.869 0 1.216.662 1.216 1.634v3.865h2.401V9.25c0-2.22-1.184-3.252-2.764-3.252-1.274 0-1.845.7-2.165 1.193v.025h-.016a5.54 5.54 0 0 1 .016-.025V6.169h-2.4c.03.678 0 7.225 0 7.225h2.4z",
    text: "in/fajriyan/",
    link: "https://www.linkedin.com/in/fajriyan/",
  },
  {
    svg: "M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.012 8.012 0 0 0 16 8c0-4.42-3.58-8-8-8z",
    text: "github/fajriyan",
    link: "https://github.com/fajriyan",
  },
  {
    svg: "M8 0C5.829 0 5.556.01 4.703.048 3.85.088 3.269.222 2.76.42a3.917 3.917 0 0 0-1.417.923A3.927 3.927 0 0 0 .42 2.76C.222 3.268.087 3.85.048 4.7.01 5.555 0 5.827 0 8.001c0 2.172.01 2.444.048 3.297.04.852.174 1.433.372 1.942.205.526.478.972.923 1.417.444.445.89.719 1.416.923.51.198 1.09.333 1.942.372C5.555 15.99 5.827 16 8 16s2.444-.01 3.298-.048c.851-.04 1.434-.174 1.943-.372a3.916 3.916 0 0 0 1.416-.923c.445-.445.718-.891.923-1.417.197-.509.332-1.09.372-1.942C15.99 10.445 16 10.173 16 8s-.01-2.445-.048-3.299c-.04-.851-.175-1.433-.372-1.941a3.926 3.926 0 0 0-.923-1.417A3.911 3.911 0 0 0 13.24.42c-.51-.198-1.092-.333-1.943-.372C10.443.01 10.172 0 7.998 0h.003zm-.717 1.442h.718c2.136 0 2.389.007 3.232.046.78.035 1.204.166 1.486.275.373.145.64.319.92.599.28.28.453.546.598.92.11.281.24.705.275 1.485.039.843.047 1.096.047 3.231s-.008 2.389-.047 3.232c-.035.78-.166 1.203-.275 1.485a2.47 2.47 0 0 1-.599.919c-.28.28-.546.453-.92.598-.28.11-.704.24-1.485.276-.843.038-1.096.047-3.232.047s-2.39-.009-3.233-.047c-.78-.036-1.203-.166-1.485-.276a2.478 2.478 0 0 1-.92-.598 2.48 2.48 0 0 1-.6-.92c-.109-.281-.24-.705-.275-1.485-.038-.843-.046-1.096-.046-3.233 0-2.136.008-2.388.046-3.231.036-.78.166-1.204.276-1.486.145-.373.319-.64.599-.92.28-.28.546-.453.92-.598.282-.11.705-.24 1.485-.276.738-.034 1.024-.044 2.515-.045v.002zm4.988 1.328a.96.96 0 1 0 0 1.92.96.96 0 0 0 0-1.92zm-4.27 1.122a4.109 4.109 0 1 0 0 8.217 4.109 4.109 0 0 0 0-8.217zm0 1.441a2.667 2.667 0 1 1 0 5.334 2.667 2.667 0 0 1 0-5.334z",
    text: "fajriyan.nur",
    link: "https://instagram.com/fajriyan.nur",
  },
];
